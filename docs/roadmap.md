...
| D40      | **`router.refresh()` does not apply the payload it fetched. App-wide, not one screen.** Found 2026-08-28 on the expenses screen; **narrowed 2026-08-31**; **re-measured and re-scoped 2026-09-21**, against a real production build (`next build`, then the standalone `server.js` the image actually runs) driven by Chromium. It is not a money bug and not a server bug: every write lands and the server renders the right thing — a manual RSC fetch contains the new row, plain HTML contains it, a client-side navigation away and back shows it, and a full reload shows it. Only `router.refresh()` fails to apply what it fetched. **The measurement, 2026-09-21:** recording an expense, the refresh applied in **0 of 5** runs while the server held the row in 5 of 5; creating a brand, **2 of 6**; creating a category, **4 of 6** — same build, same session, same browser. **This row previously read "Not app-wide … specific to this screen", and that was wrong.** It was inferred from the write-off and returns specs passing, which they do because they are slow enough to land inside the window; it is bimodal, not screen-bound. [D77](#known-defects) is the same defect on `/admin/purchases/[id]` and the stock-count sheet is a fourth — see the 2026-09-21 verification log. **Ruled out by experiment:** the CSP middleware (bypassed for RSC requests, then removed entirely), `next start` versus the standalone server, a stale API response, and the page's query string. **Not yet tested:** `RouteTransitionProvider`, which wraps navigations in `useTransition` and installs a document-level capture-phase click listener, and the `<PendingRegion key={pathname}>` in `shell.tsx` that every admin page renders inside. Both are app-wide and neither appears in any ruled-out list | `apps/web/src/lib/navigation/route-transition.tsx`, `apps/web/src/components/admin/shell.tsx`, 63 `router.refresh()` call sites | Every admin write screen can show stale state after a successful write. It is also the one thing keeping E2E off a production build in CI |
...
| # | Item | Why now |
|---|---|---|
| 1 | **[D40](#known-defects) — `router.refresh()` applies the payload it fetched, or does not** | **Re-scoped 2026-09-21 and now the clear first item.** It is not one screen: measured against a real production build, the refresh applied 0/5 on `/admin/expenses`, 2/6 on brands, 4/6 on categories, and failed on the stock-count sheet — which goes on offering *Apply to stock* after applying it. **63 call sites** across the admin depend on it, one already carries a `window.location.reload()` workaround ([D77](#known-defects), the same defect), and it is the only thing keeping E2E off a production build in CI — where a production build already passes 40/42. Five hypotheses are ruled out, so the next is not cheap; two app-wide ones have never been tested — `RouteTransitionProvider`'s `useTransition` wrapper and the `<PendingRegion key={pathname}>` every admin page renders inside |
