...
Last updated: **2026-09-09**. Phone numbers are now stored one way (D48), which is the last defect
that could silently corrupt business data. Before that: the walk-in and media defects on 09-01, the
POS customer lookup on 09-03, and printable barcode labels on 09-04. Phases 37 and 38 shipped on
08-31, when VAT became an editable setting, the last two API-only areas got their screens, and the
E2E suite went into CI.
...
| 06  | Inventory engine                      | ✅      | 🟡       | Ledger, reservations, transfers, WAC,`verify_inventory`. The product form can now open stock and write a reasoned adjustment per variant, and `/admin/inventory` carries a write-off panel. Stock counts and transfers got their screens with phase 39. What is still missing is a stock **adjustment** from the inventory screen itself, which remains read-only for that one action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
process gaps. D1, D2, D3, D5, D10, D11, D12, D13, D16, D17, D43, D44, D45, D46 and D48 have since been fixed and are struck through.
...
Phases 35–39 are complete. Every admin screen exists, and as of 2026-09-09 no open defect can
silently corrupt business data. The remaining work is no longer *building* — it is the things that
need a decision, a provider, or an environment.
