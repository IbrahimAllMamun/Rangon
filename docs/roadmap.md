...
process gaps. D1, D2, D3, D5, D10, D11, D12, D13, D16, D17, D43, D44 and D46 have since been fixed and are struck through.
...
| D45      | **Concurrent counter sales can duplicate the walk-in customer.** `orders/services/pos.py` resolves it with `get_or_create(is_walk_in=True, name=...)`, and no unique constraint backs that lookup — so `get_or_create` is not atomic. Six simultaneous POS sales made two rows and `test_simultaneous_sales_all_land_in_one_drawer` failed with `MultipleObjectsReturned`. Intermittent: it needs a loaded machine to open the window (that run took 21m45s against the usual ~5m). **Not fixed** | `apps/api/orders/services/pos.py:72`, `apps/api/customers/models.py` | Worse than a flaky test — once two rows exist, *every* later anonymous counter sale at that branch fails until one is deleted by hand |
...
| D47      | **Two pytest runs on one machine corrupt each other.** `docker-compose.test.yml` pins `name: rangon-test` and `config/settings/test.py` pins `TEST["NAME"] = "rangon_test_db"`, so every concurrent run shares one PostgreSQL *and* one test database. pytest-django drops and recreates it at session start, so a second run pulls the database out from under a first one. Observed 2026-09-01 while diagnosing D46, from a second Claude session in another worktree: one run died with `database "rangon_test_db" does not exist` mid-suite, and an earlier one reported `assert '2000.00' == '1000.00'` in `TestEndpoint::test_money_leaves_as_strings` — the other session's committed rows, read straight through. **Not fixed.** Workaround: give each run its own project, `docker compose -p <unique> -f docker-compose.test.yml run --rm -T api-test pytest -q` (the image rebuild is cache-warm, ~30 s) | `docker-compose.test.yml`, `apps/api/config/settings/test.py:33` | Worse than a flaky test: it manufactures *plausible* failures in unrelated assertions, which is exactly the kind of evidence a diagnosis session will chase. Both symptoms above were initially read as regressions from the change under test |
| D48      | **The same customer can be filed twice under two spellings of one phone number.** Found 2026-09-03 while building the POS customer lookup. There is no phone normalisation anywhere: `Customer.save()` maps empty to NULL and nothing else, `phone` is `unique=True` on the raw string, and checkout matches a returning guest with an exact `filter(phone=phone)`. So `01712345678` and `+8801712345678` are two rows for one person. The counter lookup survives this for Bangladeshi numbers only by coincidence — `+880` followed by the subscriber digits *contains* the local `0`-prefixed form, so `phone__icontains` matches — but the reverse does not, and nor does any other format the storefront lets a customer type into checkout. **Not fixed**; the shape of the fix is a normalised column written on save, backfilled, and used by both the lookup and checkout's guest match | `apps/api/customers/models.py`, `apps/api/orders/services/checkout.py:469`, `apps/api/customers/api/views.py` | Breaks phone-first identity (business rules §6), which exists to stop exactly this. A returning customer's history splits across two records, so lifetime spend, loyalty and the party ledger all under-report — and the cashier who cannot find someone is the one who creates the second record |
