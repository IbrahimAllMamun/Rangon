...
| ~~D79~~ | ~~**A column nothing could write: every purchase order ever raised carried `tax_total 0.00`.**~~ **Fixed 2026-09-18.** `PurchaseOrderItem.tax_rate` has existed since the first migration and `recalculate_totals` has always read it — but the `PurchaseLine` dataclass had no such field, the create serializer had no such field, and the viewset never passed one. Nothing at any layer could set it, so the VAT a supplier charged was unrecordable and every purchase order carried no tax at all. Invisible for as long as it existed, because nothing read the figure back: the VAT return is what finally asked the question, and a return shipped on that data would have told the owner they owed the full output VAT with nothing to reclaim — worse than no report. The chain is wired end to end and the purchase order form asks for one VAT percentage per order, stored per line where the column is, so a mixed-rate order needs no migration later | `apps/api/purchasing/services.py`, `apps/api/purchasing/api/serializers.py`, `apps/api/purchasing/api/views.py`, `apps/web/src/components/admin/purchase-order-form.tsx` | No migration — every column already existed. A model field with no writer is invisible to a type check, a lint and a test suite alike; only a caller that needs the value finds it |
| ~~D80~~ | ~~**Changing the VAT setting left the storefront quoting the old treatment.**~~ **Fixed 2026-09-18.** Measured in a real browser, three changes eleven seconds apart, and the product page showed the *previous* setting every time — EXCLUSIVE 15% still read as 0%, INCLUSIVE 15% still read `+ 15% VAT`, and 0% still read `+ 15% VAT`. The page caches against the `products` tag with a 60-second ISR window and nothing busted it, because until the price note existed the VAT setting touched no storefront page. `update_tax_settings` now pings the storefront the way a menu edit does ([content/tasks.py](../apps/api/content/tasks.py)), and only when a value actually changed, so re-saving rebuilds nothing | `apps/api/accounts/services.py` | Found the same way [D74](#known-defects), [D76](#known-defects) and [D77](#known-defects) were: by driving the real browser. A cache that was correct for years became wrong the moment a new page started reading the value |
...
| 2 | **Settle VAT** (D-C) | Orders freeze the treatment they were priced under. The default is exclusive at 0% — a placeholder, not an answer — and no later setting change corrects an order already taken. **Nothing else is now waiting on it**: as of 2026-09-18 both treatments are implemented, refunds carry the tax, memos stay silent at zero, the storefront says which treatment a price was quoted under, a supplier's VAT is recordable, and `/admin/reports/vat` files the return. It is one number in one field | Owner's answer |
