...
**D40 is not what it looked like.** Five hypotheses ruled out by experiment; see the entry. The short
version: the money is right, the server is right, and only `router.refresh()` fails to apply what it
fetched — on this one screen, in a production build.
