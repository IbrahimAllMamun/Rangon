...
production" ([D80](#known-defects)). Production settings now refuse the seed unless the operator opts
in for the one command and brings a password of their own, and the rebuild script stops before its
teardown if none is set.

**Goods now arrive through purchasing, and purchasing can do the whole job.** On 09-17 the owner
observed that adding a product and raising a purchase order did the same job twice; that pass fixed the
costing underneath (D72, D73) and left the restructure itself for later. It is built. On
`/admin/purchases/new` a product nothing in the catalogue describes is made on the order — name,
category, the sizes or colours that came, a selling price and the cost, product and variants in one
transaction — and arrives as lines at that cost. Goods that came with the supplier are received in the
same step that records them (`receive_now`: created, sent and received, or none of the three). And a
line is priced at what *this* supplier was last paid, derived from receipts rather than stored, because
a stored supplier price would drift from the deliveries it summarises. The product form keeps
photographs, descriptions and specifications, and says so. See
[§ 4.0b of business-rules.md](business-rules.md#40b-goods-arrive-through-purchasing).

**Auditing `purchase-orders/` before building over it found five defects** — the tenth time that habit
has paid. Every guard was seen to fail against the original code first; the failures were all the
defect itself (201s that should have been refusals, `{'cancel': 'ok', 'receive': 'ok'}` from the race):
a purchase order with money paid against it could be cancelled, after which the payment appeared on no
list anywhere ([D75](#known-defects)); cancel and send decided against a stale copy, so a cancel could
land on an order being received ([D76](#known-defects)); negative shipping, a discount above its line and
a variant named twice were stored ([D77](#known-defects)); one line named twice in a delivery kept only
its last quantity ([D78](#known-defects)); and the product form's own `generate-variants/` built SKUs on a
specification attribute and skipped values it did not know ([D79](#known-defects)).

**And the order screen's inline "New supplier" had never worked** ([D81](#known-defects)). It was found
the way this file keeps saying things are found — by driving the page, not by reading it. The code
read as a double submit; the browser showed something worse: a `<form>` nested inside the order's
`<form>` never reaches React's `onSubmit` at all, so "Create supplier" fell through to the browser's
native GET submission, reloaded the page and threw away the order being typed. The new-product panel
had the same shape and would have done the same. The order is no longer a `<form>`.

Before that, **2026-09-17**. Two passes on 09-17.
...
| 07  | Suppliers + purchasing                | ✅      | ✅       | PO → receive → ledger → cost recalculation. **The origin of goods since 2026-09-19** — a product is made on the order, goods that arrived are received in the same step, and lines are priced at the supplier's last price ([business-rules §4.0b](business-rules.md#40b-goods-arrive-through-purchasing)).**Admin screens shipped 2026-08-22** — `/admin/purchases/new` (supplier picker with inline create, debounced variant search, line table, live totals), `/admin/purchases/[id]` (send, cancel, partial receive, delivery history) and `/admin/suppliers` (list + inline create/edit). Receiving is the only step that writes stock, and it goes through `inventory.services`                                                                                                                                                                                                                                                                                                                                                                                  |
...
### Goods through purchasing, and the seed guard, 2026-09-19

Every new guard was **seen to fail before it was trusted**. The purchase-order defect tests were run
against the original code by swapping the six fixed files back to `HEAD`: ten of eleven failed, each
with the defect itself — `201` where a refusal belonged, a bare `409` or `404` where a field error
belonged, `200` for cancelling a paid order, and `{'cancel': 'ok', 'receive': 'ok'}` from the race.
The eleventh is the control (a discount equal to its line is allowed) and passed, as it should. The
seed guards were sabotaged one at a time instead, since their tests import code that did not exist:
guard moved after `--reset`, rotation removed, README password accepted. **The first sabotage stayed
green** — `handle()` is one transaction, so a late refusal rolled the reset back and the data test
could not tell the order apart. A spy on `_reset` now pins it, and goes red.

```text
pytest (full suite, container) ........ 1038 passed in 381s
  of which new ........................ 15 seed guard, 8 purchase-order guards, 14 restructure
                                        (2 of them D79), 1 threaded (cancel vs receive,
                                        interleaving forced)
vitest ................................ 217 passed, 15 files (8 new: lineFromVariant,
                                        repriceForSupplier)
ruff check + ruff format --check ...... clean, 204 files
tsc --noEmit + eslint ................. clean
makemigrations --check --dry-run ...... no changes detected (no schema change in either)
rebuild-local-prod.sh ................. both refusals fire before Docker is touched
```

**The browser walk found D81 and has not yet passed end to end.** `e2e/purchase-origin-walk.mjs`
drove Edge against the dev stack: sign-in, the product form's pointer to purchasing and the new
order screen all passed, and then "Create supplier" navigated to `/admin/purchases/new?`. Probing
the live page showed the nested form's React handler never fired. The fix (the order is no longer a
`<form>`) is typechecked and linted; the first re-run stalled because the dev server had not picked
the change up through the Windows bind mount (`.claude/environment.md` §13's shape, for an edited
file this time), and the run after the container restart was interrupted. **Do not describe the new
purchase-order screen as browser-verified until that script passes.**
...
| ~~D47~~ | ~~**Two pytest runs on one machine corrupt each other.**~~ **Fixed 2026-09-15 — see the row below for how, and for the fix that looked right and was not.** Originally: `docker-compose.test.yml` pins `name: rangon-test` and `config/settings/test.py` pins `TEST["NAME"] = "rangon_test_db"`, so every concurrent run shares one PostgreSQL *and* one test database. pytest-django drops and recreates it at session start, so a second run pulls the database out from under a first one. Observed 2026-09-01 while diagnosing D46, from a second Claude session in another worktree: one run died with `database "rangon_test_db" does not exist` mid-suite, and an earlier one reported `assert '2000.00' == '1000.00'` in `TestEndpoint::test_money_leaves_as_strings` — the other session's committed rows, read straight through. **Not fixed.** Workaround: give each run its own project, `docker compose -p <unique> -f docker-compose.test.yml run --rm -T api-test pytest -q` (the image rebuild is cache-warm, ~30 s) | `docker-compose.test.yml`, `apps/api/config/settings/test.py:33` | Worse than a flaky test: it manufactures *plausible* failures in unrelated assertions, which is exactly the kind of evidence a diagnosis session will chase. Both symptoms above were initially read as regressions from the change under test |
...
| ~~D75~~ | ~~**A purchase order with money paid against it could be cancelled, and the money vanished from the books.**~~ **Fixed 2026-09-19.** `cancel_purchase_order` checked for receipts and nothing else. The payables selector (business-rules §4.2) drops a `CANCELLED` order and a supplier payment can be neither edited nor deleted (§6b.1b), so an advance of 400 against a sent order was, after a cancel, no longer owed, no longer payable and on no list anywhere as money the supplier holds. Refused with `CONFLICT` now; until a supplier credit note exists, an order with money against it is received, not cancelled. A second cancel of a cancelled order — which used to succeed and write a second audit entry — is refused too | `apps/api/purchasing/services.py` | Found by auditing `purchase-orders/` before the purchasing restructure was built over it — the tenth time that habit has paid |
| ~~D76~~ | ~~**Cancel and send decided against the caller's copy of the order.**~~ **Fixed 2026-09-19.** `cancel_purchase_order` read the receipts and then wrote the status with no lock between them. A delivery that had locked the order and written its receipt but not committed was invisible to that read, so the cancel found "no receipts", queued behind the delivery's row lock and, once it committed, overwrote `RECEIVED` with `CANCELLED` — stock on the shelf against an order that says it was never placed, and off the payable list. `send` had the same shape against a cancel. Both now take `select_for_update` on the order first — the lock receiving and paying already take — and decide under it. The regression test forces the interleaving by holding the delivery open at `receive_stock` rather than hoping for it, and reproduced `{'cancel': 'ok', 'receive': 'ok'}` on the old code | `apps/api/purchasing/services.py`, `apps/api/tests/test_concurrency.py` | — |
| ~~D77~~ | ~~**A purchase order stored money that could not be right.**~~ **Fixed 2026-09-19.** `shipping_total` had no floor, so 10,000 of goods with shipping `-500.00` was stored as a 9,500 liability; a line discount had no ceiling, so 2 × 100 less 500 stored a line of −300 that silently cancelled out other lines; the same variant on two lines reached `purchasing_poi_uniq` as a bare 409 with no field; and an unknown supplier came back as a 404 for the page. The order form had been catching the first three client-side all along — its own comment said "a duplicate variant fails with an opaque 409" — while the API took whatever it was sent. `purchasing.services` now refuses all four as `VALIDATION_ERROR` with a field in `details`, so the shell, the seed and the importer meet them too | `apps/api/purchasing/services.py`, `apps/api/purchasing/api/serializers.py` | CLAUDE.md §3.4: the frontend may hide a mistake; the API must still refuse it |
| ~~D78~~ | ~~**One order line named twice in a delivery kept only its last quantity.**~~ **Fixed 2026-09-19.** The receive view folded the lines into a dict keyed by item, so `[{item, 3}, {item, 4}]` received 4 — not 7, and not an error. The storekeeper counted seven onto the shelf and the ledger said four. Refused now | `apps/api/purchasing/api/serializers.py` | — |
| ~~D79~~ | ~~**`generate-variants/` built SKUs on a specification, and skipped values it did not know.**~~ **Fixed 2026-09-19.** business-rules §5a's first rule — an attribute cannot be both an axis and a specification — was enforced only on the specification side, so `{"material": ["Cotton"]}` made variants on Material while the same product could state Material as a spec: the one state §5a exists to make impossible, and the one rule 2's side-change guard assumes cannot happen. And values were matched with `value__in`, erroring only when *none* matched, so `S` and `XXXL` made S alone and said nothing. Both refused; the whole request or none of it. The new `products/quick-create/` goes through the same function, so it inherits the guards rather than the holes | `apps/api/catalog/services.py` | — |
| ~~D80~~ | ~~**`seed_demo` would put the README's password on a production database.**~~ **Fixed 2026-09-19.** `PASSWORD = "rangon12345"` carried the comment "development seed only — never reaches production", and nothing enforced it: `scripts/rebuild-local-prod.sh` ran `seed_demo --reset` against `config.settings.prod` on every rebuild, so owner, manager, cashier, stock and accounts all opened with a password printed in the public README — on a stack that has been published through the Cloudflare tunnel. `--reset` also deletes every order, payment and ledger row, which a production database should not answer to a typo. A `DEMO_SEED` setting now decides: `development` (base) keeps the README password for a laptop; `prod.py` sets `off` unless `DJANGO_ALLOW_DEMO_SEED=1`, and then only with a `DJANGO_DEMO_SEED_PASSWORD` that is not the README one and passes the password validators. The refusal comes before `--reset` is attempted — not merely rolled back by the command's transaction, which is what a first version of the test could not tell apart. A re-seed with a password of its own also replaces the README one on any account an earlier seed left with it, audited; a password someone rotated by hand is left alone. The rebuild script passes the opt-in to the one `exec` and stops **before** `compose down -v` when no password is configured | `apps/api/core/management/commands/seed_demo.py`, `apps/api/config/settings/`, `scripts/rebuild-local-prod.sh` | A database seeded before 2026-09-19 still carries the public password until it is reseeded with one of your own or rotated — [cloudflare-local-setup.md §12](operations/cloudflare-local-setup.md) |
| ~~D81~~ | ~~**The purchase order's inline "New supplier" reloaded the page and lost the order.**~~ **Fixed 2026-09-19.** `SupplierForm` renders its own `<form>` inside `PurchaseOrderForm`'s `<form>`. In this app a nested form never reaches React's `onSubmit`: dispatching `submit` at the inner form left `defaultPrevented` false while the same dispatch at the outer form was handled, and a real click on "Create supplier" navigated to `/admin/purchases/new?` — the browser's native GET submission of a form whose inputs have no `name`. No supplier was created, the page reloaded, and every line already on the order was gone. Read from the code it looked like a double submit; only the browser showed what it was. Nested forms are invalid HTML and React warns about them in the console, which nobody was reading. The order is now a plain container saved by its button's click, so both inline panels — the supplier form and the new product panel, which had the same shape — are top-level forms. A side effect worth having: Enter inside an order line no longer submits a purchase order | `apps/web/src/components/admin/purchase-order-form.tsx` | Found by the browser walk of the purchasing restructure; `e2e/purchase-origin-walk.mjs` asserts no navigation to `…?`. **The fix itself has not yet been seen passing in a browser** — the re-run was interrupted |
...
| `auth/password/change/` | **Not previously recorded.** This section's 09-15 audit swept router registrations and so missed the `auth/` sub-routes. An owner or admin can reset anyone's password from `/admin/staff`, including their own, so nothing is stuck — but a cashier or accountant who suspects theirs is compromised has to ask one |
...
| — | **Goods arrive through purchasing** (the 09-17 observation) | 2026-09-19 — found D75–D79 and D81 on the way; browser walk still to pass |
...
| # | Item | Why now |
|---|---|---|
| 1 | **[D40](#known-defects) — `router.refresh()` in a production build** | The only thing keeping E2E off a production build in CI. Narrowed to one sentence with five hypotheses already ruled out — which is also a warning that the next one is not cheap either |
| 2 | **Media library** | Worth having once there is a real photo library to manage. Before Tier 0 #3 there is nothing to organise |
| 3 | **A reader for `audit-logs/` and `inventory-transactions/`** | Both from the API-only audit. Everything writes audit rows and nothing reads them back, so the trail exists and is unreadable without database access; `/admin/inventory` shows the current figure and not the movements behind it |
| 4 | **Password self-service** (`auth/password/change/`) | Newly recorded. An owner can reset anyone's password from `/admin/staff`, so nothing is stuck — but a cashier who suspects theirs is compromised has to ask one |
