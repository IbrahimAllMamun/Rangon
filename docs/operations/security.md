...
| XSS | React escapes by default. **One** `dangerouslySetInnerHTML`, in `components/commerce/rich-text.tsx`, renders site-page bodies (About, the policies, the shop's own pages) — and only what the API stored, which it sanitises **on write** with `nh3` (Rust's `ammonia`) to the tags the admin editor can produce: `p br h2 h3 strong em u s a ul ol li blockquote hr`, `href` only, schemes `http https mailto tel` plus relative. Scripts, styles, event handlers, images and iframes never reach the table, whatever is PATCHed ([ADR-0012](../architecture/decisions/0012-storefront-footer-and-site-pages.md), `tests/unit/test_site_content_rules.py`). CSP sent by the web app itself (`apps/web/src/middleware.ts`) with a per-request nonce — **not** by Nginx, which would append a second policy and block the nonced scripts. **Measured 2026-09-24** on a production build: every document route, a 404 and the redirects included, sends it; every `<script>` carries the nonce; the nonce is new per request; `e2e/csp.spec.ts` checks in a real browser that each page's scripts load under it |
...
| Headers | `X-Content-Type-Options`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-Frame-Options: DENY`, CSP with `'nonce-…' 'strict-dynamic'` and no `unsafe-inline`/`unsafe-eval` in `script-src`. **`frame-src https://www.google.com` and nothing else** (2026-09-26), for the Contact page's map — the one third party framed into the shop, sandboxed without `allow-top-navigation`. The origin must match `MAP_EMBED_ORIGIN` in `content/validators.py`, the only origin a map may be saved with |
| Links and frames an admin types | Every `href` or iframe `src` that comes from a form is reduced to a safe shape on the server (`content/validators.py`). Navigation and footer links: a site path, `http(s)://`, `mailto:` or `tel:` — `javascript:`, `data:`, `//host` and `/\host` are refused. Social profiles: `https://` on the platform's own domain, no credentials or port. Maps: Google's `<iframe>` code is accepted and **only its `src` kept**, which must be `https://www.google.com/maps/embed…`; pasted markup is never stored or rendered |
...
| Stored XSS through a site page | Page bodies are HTML from a rich-text editor, rendered on every storefront visit. The editor is a convenience, not the control: anyone holding a token can PATCH raw HTML, so the API sanitises every body before storing it, and the storefront renders only the stored value. Unpublishing is how a page is withdrawn; nothing is rendered from a copy baked into the build |
| A hostile link or frame planted through the footer | A manager's account is enough to edit the footer, so its fields are treated as untrusted: link schemes are allow-listed, social links must stay on their platform's domain, and a map can only be Google's embed — the CSP `frame-src` refuses any other frame even if a bad value got through |
